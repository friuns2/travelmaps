  ...
  <div class="bg-white bg-opacity-10 rounded-lg overflow-hidden shadow-lg transform hover:scale-105 transition-transform duration-300 cursor-pointer"
    ...
    <div class="p-4">
      ...
      <button @click.stop="toggleAttractionInItinerary(attraction)"
        ...
        {{ attraction.inItinerary ? 'Remove from Itinerary' : 'Add to Itinerary' }}
      </button>
    </div>
  </div>
</template>
...
const toggleAttractionInItinerary = (attraction) => {
  ...
  state.value.map.panTo(attraction.location)
  const marker = state.value.markers.find(marker => marker.getPosition().equals(attraction.location))
  marker.setIcon({
  ...
  if (!attraction.inItinerary)
    calculateDirections()
  if (!state.value.attractions.some(a => a.inItinerary)) {
    state.value.activeView = 'map'
  }
}
